...
export default function PurchaseInvoices() {
  ...
  const { isAdmin, isAccounts, hasWarehouseAccess, warehouses } = useAuth();
  ...
  return (
    <div className="w-full min-w-0 max-w-full overflow-x-hidden px-2 sm:px-4 lg:px-6 py-3 sm:py-6 space-y-3 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {(isAdmin || isAccounts) && (
          <Button onClick={() => navigate(`${moduleBase}/purchase-invoices/new`)}>
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem onClick={() => navigate(`${moduleBase}/purchase-invoices/${invoice.id}`)}>
                                  <Eye className="h-4 w-4 mr-2" />
                                  View Details
                                </DropdownMenuItem>
                                {isAdmin && invoice.status !== 'paid' && invoice.status !== 'cancelled' && (
                                  <DropdownMenuItem onClick={() => navigate(`${moduleBase}/purchase-invoices/${invoice.id}/edit`)}>
                                ...
                                {balance > 0 && (isAdmin || isAccounts) && (
                                  <DropdownMenuItem 
                                    onClick={() => navigate(`${moduleBase}/supplier-payments/new?invoice=${invoice.id}`)}
                                  >
